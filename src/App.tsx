...
import { calculateSalaryToHourly } from './logic/salaryToHourlyCalculations';
import type { SalaryToHourlyInput } from './logic/salaryToHourlyCalculations';

function App() {
  const [values, setValues] = useState<SalaryToHourlyInput>({
    annualSalary: 60000,
    hoursPerWeek: 40,
    weeksPerYear: 52
  });

  const handleChange = (field: keyof SalaryToHourlyInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateSalaryToHourly(values);
